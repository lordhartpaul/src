...
// GUI objects -- Should be private unless absolutely necessary
  ...
  private Scrollbar animslider;
  private PhaseThread phaseThread;
...
class MoonCanvas extends Canvas {
  ...
  protected void paintSun(Graphics g) {
    int size = (int)((double)d.width*scale*top.sunsize/2);

    g.setColor(Color.yellow);
    g.fillOval(xmid - size - (int)((double)d.width*scale*top.sundist), 
	ymid - size, size*2, size*2);
  }

  protected void paintMoon(Graphics g) {
    int size = (int)((double)d.width*scale*top.moonsize/2);

    g.setColor(Color.darkGray);
    g.fillOval(xmid - size + (int)((double)d.width * scale * top.moondist * 
		Math.cos(top.phase*2.0*Math.PI)), 
	ymid - size - (int)((double)d.width * scale * top.moondist * 
		Math.sin(top.phase*2.0*Math.PI)), size*2, size*2);
    g.setColor(Color.white);
    g.fillArc(xmid - size + (int)((double)d.width * scale * top.moondist * 
		Math.cos(top.phase*2.0*Math.PI)), 
	ymid - size - (int)((double)d.width * scale * top.moondist * 
  ...
  protected void paintEarth(Graphics g) {
    int size = (int)((double)d.width*scale*top.earthsize/2);
...
class MoonCanvas2 extends Canvas {
  ...
  protected void paintSun(Graphics g) {
    int size = (int)((double)d.width*scale*extrasun/2);

    g.setColor(Color.yellow);
    g.fillOval(xmid - size - (int)((double)d.width * scale * 
	(top.phase-0.5) * 720), ymid - size, size*2, size*2);
  }

  protected void paintMoon(Graphics g) {
    int i, limit, sign;
    int size = (int)((double)d.width*scale/2);
    double p = top.phase;
...
//    sign = 1;
    ...
    for (i = -limit; i <= limit; i++)
      g.drawLine(xmid + sign * (int)((double)limit * 
		Math.sqrt(1.0-(double)(i*i)/((double)(limit*limit)))),
		ymid + i,
		xmid + sign * (int)((double)limit * 
